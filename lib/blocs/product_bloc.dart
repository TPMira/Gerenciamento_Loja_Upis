...
class ProductBloc extends BlocBase {
  ...
  ProductBloc({this.categoryId, this.product}){
    if(product != null){
      ...
      unsavedData["types"] = List.of(product.data["types"]);

      _createdController.add(true);
    }else{
      unsavedData = {
        "title": null, "description": null, "price": null , "image":[], "types":[]
      };
    ...
    try{
      if(product != null){
       ...
       await product.reference.updateData(unsavedData);
        ...
        DocumentReference dr = await Firestore.instance.collection("products").document(categoryId).collection("items").add(Map.from(unsavedData)..remove("image"));
  ...
  Future _uploadImages(String productId) async{
    for(int i = 0; i < unsavedData["image"].length; i++) {
      ...
      StorageUploadTask uploadTask = FirebaseStorage.instance.ref().child(
          categoryId).child(productId).child(DateTime
          .now()
          .millisecondsSinceEpoch
          .toString()).putFile(unsavedData["image"][i]);
