...
class UserBloc extends BlocBase {
  ...
  void _subscribeToOrders(String uid){
    _users[uid]["subscription"] = Firestore.instance.collection("users").document(uid).collection("orders").snapshots().listen((orders)async{

      int numOrders = orders.documents.length;
      double money = 0.0;
      
      for(DocumentSnapshot d in orders.documents){
        DocumentSnapshot order = await Firestore.instance.collection("orders").document(d.documentID).get();

        if(order.data == null) continue;

        money += order.data["productsPrice"];
      }
